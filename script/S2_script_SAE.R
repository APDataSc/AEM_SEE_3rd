...
## Limpiando la memoria de R y fijando la cantidad de dígitos
rm(list = ls())
#options(digits = 2)
...
### Cargue de la muestra y las covariables necesarias para el proceso de estimación
...
### Construyendo la tabla con la información para el cálculo del estimador
...
## el Error Cuadrático Medio (ECM) y se calcula el Error Cuadrado Medio Relatico (ECMR)
## El cual es una medida de calidad para comparar la estimación directa con la
...
## Se construye una tabla de datos con la información de los resultados obtenidos
...
Estima_SAE_obs<- data.frame(Dominio = area_values$Dominio,
                            ...
                            ECMR = ECMR)
head(Estima_SAE_obs)
## Construyamos una tabla para los dominios no muestreados. Primero, se deben extraer los betas
...
## Construyendo la tabla de datos con las predicciones para los dominios no observados
...
## Generar los insumos para el cálculo del ECM en los dominios no observados
...
## Para generar la varianza de los betas, se organiza la información auxiliar en 
...
## Una vez se tiene la estimación de la varianza de los betas, se organiza la información auxiliar 
...
## Cálculo de los Errores estándares relativos
ECMR_Nobs<- 100*sqrt(ECM_Nobs)/predic_SAE_Nobs$Est_Eblup

## Agregar a la tabla de datos de predic_SAE las columnas ECM y ECMR
predic_SAE_Nobs$ECM<- ECM_Nobs
predic_SAE_Nobs$ECMR<- ECMR_Nobs

## Agregar a la tabla de datos global, todas las estimaciones con el modelo
## Fay Herriot
Base_completa_SAE<- rbind(Estima_SAE_obs, predic_SAE_Nobs)
dim(Base_completa_SAE)

## Se genera una tabla de datos completa que contenga la información de la estimación
